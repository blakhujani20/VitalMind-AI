...
if activity_file and sleep_file:
    try:
        activity_df_uploaded = pd.read_csv(activity_file)
        sleep_df_uploaded = pd.read_csv(sleep_file)
        st.sidebar.success("Files uploaded! Processing your data.")
        final_df = run_preprocessing_pipeline(activity_df_uploaded, sleep_df_uploaded)
    except Exception as e:
        st.sidebar.error(f"Error reading files: {e}")
else:
    st.sidebar.info("Using default Kaggle dataset. Upload your own files to analyze your personal data.")
    activity_df_default, sleep_df_default = load_raw_data(DATA_PATH)
    if activity_df_default is not None:
        final_df = run_preprocessing_pipeline(activity_df_default, sleep_df_default)

if final_df is not None:
    prediction_model = load_prediction_model(MODEL_PATH)
    scaler = load_scaler(SCALER_PATH)
    vector_store = get_vector_store(final_df) 
    ...
    with tab1:
        ...
        col2.metric("Avg Calories", f"{final_df['Calories'].mean():.0f}")
        col3.metric("Avg Sleep (hrs)", f"{final_df['SleepHours'].mean():.1f}")
    ...
    with tab4:
        ...
        if len(data) > 14:
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=final_df['Date'], y=final_df['Steps'], mode='lines', name='Historical Data'))
            ...
            st.warning("Not enough data to generate a forecast. Please upload a dataset with at least 15 days of records.")

    with tab5:
        st.subheader("Chat with your VitalMind AI Assistant")
        assistant = HealthAssistant(vector_store, final_df)
    ...
    st.error("Could not load data. Please check for the default data files in the /data folder or upload your own.")